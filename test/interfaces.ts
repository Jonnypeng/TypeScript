...
function createMaxSquare(config: SquareConfig): { color: string; area: number } {
  let newSquare = { color: "white", area: 100 };
  if (config.clor) {
...
let mySquare = createMaxSquare({ color: "black" });
...
let squareOptions: NewSquareConfig = {show:true,s:'sss'};
